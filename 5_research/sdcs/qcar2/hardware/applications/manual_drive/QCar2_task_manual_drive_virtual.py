...
qcar_spawn = QLabsQCar2(qlabs)
qcar_spawn.spawn_degrees(location = [-20,30, 0], rotation=[0,0,-90])

qcar_spawn.possess(QLabsQCar2.CAMERA_TRAILING)
...
try:
    while elapsed_time() < simulationTime:
        ...
        if configuration == '3':
            ...
            if new and gpad.buttonLeft:
                if gpad.buttonA:
                    QCarCommand = np.array([0.2*(gpad.trigger-0.5) ,
        ...
        if new:
            ...
            print("Car Speed:\t\t\t{0:1.2f}\tm/s\nMotor throttle:\t\t\t{1:4.2f}\t% PWM\nSteering:\t\t\t{2:3.2f}\trad"
                                                            .format(linearSpeed, QCarCommand[0], QCarCommand[1]))
